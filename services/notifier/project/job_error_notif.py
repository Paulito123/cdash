...
# DE14 1005 0000 1067 5591 04 > BELADEBEXXX
    ...
    try:
        ...
        if send_message == 1:
            ...
            if int(error_count) > 0:
                ...
                type = "Notif application error success" if resp['ok'] == "True" else "Notif application error failed"
        ...
        # send_message = engine.execute(f"select case when max(created_at) > (NOW() - interval '5 minutes')::timestamp then 0 else 1 end as notif_flag from eventlog where left(type, 21) = 'Notif miners inactive'").scalar()

        if send_message == 1:
            error_count = engine.execute("with maxts as (select address, max(timestamp) as mts from chainevent group by address) select count(*) from maxts m join accountstat a on m.address = a.address where mts < (NOW() - interval '40 minutes')::timestamp and proofsinepoch < 72").scalar()
            if int(error_count) > 0:
                ...
                resp = notifier.send_message(notification).json()
                type = "Notif miners inactive success" if resp['ok'] == "True" else "Notif miners inactive failed"
                jobname = os.path.basename(__file__)
